...
/*** ADC Part ***/
assign o_AD_CLK = clk_125;
reg signed [AD_WIDTH-1:0] r_AD_DATA_A;
reg signed [AD_WIDTH-1:0] r_AD_DATA_B;

always @(posedge i_AD_DCO_A or negedge sys_rst_n) begin
    if(!sys_rst_n) begin
        r_AD_DATA_A <= 0;
    end
    else begin
...
always @(posedge i_AD_DCO_B or negedge sys_rst_n) begin
    if(!sys_rst_n) begin
        r_AD_DATA_B <= 0;
    end
    else begin
...
wire [DA_WIDTH-1:0] o_dds_out;
wire signed [DA_WIDTH-1:0] o_dds_out_signed;
reg [DA_WIDTH-1:0] r_dds_out;
reg signed [DA_WIDTH-1:0] r_dds_out_signed;
...
always @(posedge clk_125 or negedge sys_rst_n) begin
   if(!sys_rst_n) begin
       ...
       r_dds_out_signed <= o_dds_out_signed;
       r_dds_out <= o_dds_out;
   end
end

/*
wire signed [DA_WIDTH + 16 - 1:0] mul_A;
wire signed [DA_WIDTH + 16 - 1:0] mul_B;
...
always @(posedge clk_250 or negedge sys_rst_n) begin
    if(!sys_rst_n) begin
        Au <= 0;
        As <= 0;
        Bu <= 0;
        ...
        Au <= {~mul_A[DA_WIDTH + 16 - 1], mul_A[DA_WIDTH + 16 - 2:16]};
        As <= mul_A[DA_WIDTH + 16 - 1:16];
        Bu <= {~mul_B[DA_WIDTH + 16 - 1], mul_B[DA_WIDTH + 16 - 2:16]};
        Bs <= mul_B[DA_WIDTH + 16 - 1:16];
    end
end
*/
...
always @(posedge clk_250 or negedge sys_rst_n) begin
    if(!sys_rst_n) begin
        ...
        if(wave_sel == 0) begin
            o_DA_DATA_A <= r_dds_out_signed;
            o_DA_DATA_B <= r_dds_out_signed;
        end
        else if(wave_sel == 1) begin
            o_DA_DATA_A <= r_dds_out;
            o_DA_DATA_B <= r_dds_out;
        end
        else if(wave_sel == 2) begin
            o_DA_DATA_A <= As;
            o_DA_DATA_B <= Bs;
        end
        else if(wave_sel == 3) begin
            o_DA_DATA_A <= Au;
            o_DA_DATA_B <= Bu;
        end
    end
...
(
    ...
    .probe0(r_AD_DATA_A),
    .probe1(r_AD_DATA_B),
    .probe2(gpio1_out_32_gp1[0]),     // i_gate_enable
    .probe3(S_AXIS_tvalid),           // packer output
    .probe4(S_AXIS_tready),           // fifo backpressure
    .probe5(S_AXIS_tlast)             // end-of-packet
);
